...
router.post(
  ...
  authenticateToken,
  upload.fields([
    { name: "training_data", maxCount: 1 },
    { name: "game_data", maxCount: 1 },
  ]),
  async (req, res) => {
    try {
      ...
      if (!roi_np_list || !Array.isArray(roi_np_list)) {
        return res.status(400).json({
          error:
            "roi_np_list is required and must be an array of objects containing roi_id and np_id",
        });
      }

      // Validate roi_np_list structure
      const isValidRoiNpList = roi_np_list.every(
        (item) =>
          typeof item === "object" &&
      ...
      if (!isValidRoiNpList) {
        return res.status(400).json({
          error:
            "Each item in roi_np_list must contain roi_id and np_id properties",
        });
      }
      ...
      if (req.files.training_data) {
        ...
        let result = xlsxService.processXLSXFile(
          req.files.training_data[0],
          fileName,
          true
        );
        if (result.success) training_data = result.data;
        else
          return res.status(400).json({
        ...
        if (res.success) training_data = res.data;
        else
          return res.status(400).json({
      ...
      if (req.files.game_data) {
        ...
        let result = xlsxService.processXLSXFile(
          req.files.game_data[0],
          fileName
        );
        if (result.success) game_data = result.data;
        else
          return res.status(400).json({
        ...
        if (res.success) game_data = res.data;
        else
          return res.status(400).json({
      ...
      let training_data_filtered = dataManipulator.filterTrainingData(
        training_data,
        {
          trziste_ids,
          partner_ids,
      ...
      let grouped_training_data = dataManipulator.groupTrainingDataByNpRoi(
      ...
      dataManipulator.test(
        training_data_filtered,
        "training_data_filtered.xlsx",
        [15, 16]
      );
      grouped_training_data.forEach((group, i) => {
        dataManipulator.test(
          group.data,
          "grouped_training_data_" + i + ".xlsx",
          [15, 16]
        );
      });
      ...
      grouped_training_data.forEach((group, i) => {
        ...
        dataNeededToCalculate.forEach((data_point) => {
          calculated_data[data_point.label] =
            dataManipulator.calculateMedianAverageStdDev(
              group.data,
              data_point.colIndex
            );
        });
      ...
      // NEW: Analyze best performing trzista/partners for each NP/ROI group
      let best_performing_analysis = dataManipulator.analyzeBestPerforming(
      ...
      let response = {
        ...
        data: {
          ...
          best_performing_analysis: best_performing_analysis,
        },
      };
...
// NEW: Dedicated endpoint for best performing analysis
router.post(
  "/best-performing",
  authenticateToken,
  upload.fields([
  ...
  async (req, res) => {
    try {
      ...
      // Load data (same logic as calculate endpoint)
      let training_data = null;
      let game_data = null;
      ...
      // Group training data by NP/ROI groups
      let grouped_training_data = dataManipulator.groupTrainingDataByNpRoi(
        training_data_filtered,
        np_roi_filters
      );

      // Analyze best performing trzista/partners
      let best_performing_analysis = dataManipulator.analyzeBestPerforming(
        grouped_training_data,
        trziste_ids,
        partner_ids
      ...
      let response = {
        message: "Best performing analysis completed successfully",
        data: {
          best_performing_analysis: best_performing_analysis,
          analysis_summary: {
            total_groups_analyzed: best_performing_analysis.length,
            scenarios: {
              trziste_selected: trziste_ids && trziste_ids.length > 0,
              partner_selected: partner_ids && partner_ids.length > 0,
              both_selected:
                trziste_ids &&
                trziste_ids.length > 0 &&
                partner_ids &&
      ...
      console.error("Best performing API error:", error);
      res.status(500).json({
        error: "Internal server error",
        details: error.message,
      });
