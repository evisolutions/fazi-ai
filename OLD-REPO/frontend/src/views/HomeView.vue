...
const simulateCalculate = async () => {
  ...
  const roiNpList = gamba.roi_np_table_data.filter((row: any) => row.selected).map((row: any) => ({ roi_id: row.roi_id, np_id: row.np_id }));
...
const setupDataRender = () => {
  let formatted_data = result_data.value.grouped_training_data.map((group: any) => formatDataForChart(group));
...
const simulateComplete = async () => {
  ...
  try {
    const response = await axios.post(`${test.backend_url}/api/test`, 
    {
  ...
  <main>
    ...
    <v-tabs-window v-model="tab">
              ...
              <v-expansion-panel>
                ...
                <v-expansion-panel-text>
                  ...
                  <v-autocomplete
                    ...
                    label="Volatility"></v-autocomplete>
                  ...
                  <v-autocomplete
                    ...
                    label="Game Categories"></v-autocomplete>
      ...
      <v-tabs-window-item :value="2">
        ...
        <v-container class="container d-flex justify-sm-center ga-5 mt-10" fluid v-if="loading_data">
          <v-progress-circular
            color="primary"
            indeterminate
          ></v-progress-circular>
        ...
        <v-container fluid v-if="result_data">
          <div class="container mb-15" v-for="group in result_data.grouped_training_data" :key="group.id">
            <div class="d-flex align-center ga-2">
              <span>ROIS:</span>
              [
              <span v-for="(roi_id, index) in group.roi_ids">{{ gamba.getROILabelFromID(roi_id) }}{{ index !== group.roi_ids.length - 1 ? ', ' : '' }}</span>
              ]
            </div>
            <div class="d-flex align-center">
              <span>NP:</span>
            ...
            <div style="width: 800px;"><canvas :id="`test-${group.id}`"></canvas></div>
          </div>
        </v-container>
      </v-tabs-window-item>
    </v-tabs-window>
